...
data = preprocessing.scale(data)     # 标准化，尽量将数据转化为均值为零，方差为一的数据，形如标准正态分布（高斯分布）
data_ = data.reshape(data_hsi.shape[0], data_hsi.shape[1], data_hsi.shape[2])
whole_data = data_
padded_data = np.lib.pad(whole_data, ((PATCH_LENGTH, PATCH_LENGTH), (PATCH_LENGTH, PATCH_LENGTH), (0, 0)),
                         'constant', constant_values=0)   # whole_data各个维度上填补0

for index_iter in range(ITER):
    print('iter:', index_iter)
    net = network.FECNet(BAND, CLASSES_NUM)
    summary(net.to(device),input_size=(1,9,9,BAND))
    ...
    torch.save(net.state_dict(), "./net/" + Dataset + str(round(overall_acc_fdssc, 3)) + '.pt')
    KAPPA.append(kappa)
...
record.record_output(OA, AA, KAPPA, ELEMENT_ACC, TRAINING_TIME, TESTING_TIME,
                     './records/' + day_str + '_' + Dataset + 'split：' + str(VALIDATION_SPLIT) + 'lr：' + str(lr) + '.txt')

generate_png(net, Dataset, device, BAND, image_x*image_y, data_hsi, gt_hsi)
